...
for ip=1:length(imagePath)
    ...
    for im=1:numel(imgs)
        ...
        [bwTissue,bwHimg]=wsi_preprocess_bladder(RGB,thrWhite,thrOverStain);
        
        %3) get image tile locations
        %[top_left,bottom_right]=xu_SelectImageTiles_VI(bwTissue,pp,Tumor_tileSize);
        [top_left,bottom_right]=xu_SelectImageTiles_I(bwTissue,bwHimg,pp,ppt,tileSize);
