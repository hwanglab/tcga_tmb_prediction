...
function SVM_classifier_weighted_mean_transfer_2class362_lbpComparison
...
for ip=1:length(featPath)
    ...
    for im=1:numel(mats)
        ...
        coeff00=repmat(coeff,1,size(feat_out(:,1:end-1),2));
        feat_mean=sum(feat_out(:,1:end-1).*coeff00,1);
