...
function SVM_classifier_weighted_mean_transfer_2class362_unbalanced
...
featmatoutput='E:\Hongming\projects\tcga-bladder-mutationburden\Hongming_codes\step4)_wsi_classification\two_class_distinction\';
...
ComNum=70;        % number of PCA components
...
for cc=1:ItNum
    ...
    labels_h=labels_all(labels_all==3);
    feats_h=feats_all(labels_all==3,:);
    
    temp=find(labels_all==1);
    indcc=datasample(temp,length(labels_h),'Replace',false);
    labels_l=labels_all(indcc);
    feats_l=feats_all(indcc,:);
    
    feats=[feats_l;feats_h];
    labels=[labels_l;labels_h];
