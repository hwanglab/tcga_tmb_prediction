...
output_path<-"E:/Hongming/projects/tcga-bladder-mutationburden/heterogeneity/violin_log_2/"
#output_path<-'E:/Hongming/projects/tcga-bladder-mutationburden/heterogeneity/violin_zscore_2/'
...
for (kk in 1:length(pid)){
  if (plabel[kk]=="'High'-'High'-Low") {
    pid2[ind]<-pid[kk]
    plabel2[ind]<-'High-High-Low'
    ind<-ind+1
  } else if (plabel[kk]=="'High'-'High'-High" ) {
    pid2[ind]<-pid[kk]
    plabel2[ind]<-'High-High-High'
    ind<-ind+1
  } else if (plabel[kk]=="'High'-'Low'-High") {
    pid2[ind]<-pid[kk]
    plabel2[ind]<-'High-Low-High'
    ind<-ind+1
  } else if (plabel[kk]=="'High'-'Low'-Low") {
    pid2[ind]<-pid[kk]
    plabel2[ind]<-'High-Low-Low'
    ind<-ind+1
  } else if (plabel[kk]=="'Low'-'High'-High") {
    pid2[ind]<-pid[kk]
    plabel2[ind]<-'Low-High-High'
    ind<-ind+1
  } else if (plabel[kk]=="'Low'-'High'-Low") {
    pid2[ind]<-pid[kk]
    plabel2[ind]<-'Low-High-Low'
    ind<-ind+1
  } else if (plabel[kk]=="'Low'-'Low'-High") {
    pid2[ind]<-pid[kk]
    plabel2[ind]<-'Low-Low-High'
    ind<-ind+1
  } else if (plabel[kk]=="'Low'-'Low'-Low") {
    pid2[ind]<-pid[kk]
    plabel2[ind]<-'Low-Low-Low'
    ind<-ind+1
  }
  else {
    print('skip!')
...
for (cc in 1:length(pid_pdl1$COMMON)){ # consider all gene expression
...
#cc=13
  ...
  ggsave(paste(output_path,pid_pdl1$COMMON[cc],'.png'),width = 10, height = 10)
