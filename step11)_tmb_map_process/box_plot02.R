...
pid_pdl1<-read_excel(paste(data_path1,"mRNA_exp.xlsx",sep=""),sheet = "mRNA_exp")


ppdl1<-vector()
for (kk in 1:length(pid2)){
  pid_temp<-as.character(pid2[kk])
  for (jj in 1:length(pid_pdl1$SAMPLE_ID)){
    pid_samp<-as.character(pid_pdl1$'SAMPLE_ID'[jj])
    if (substring(pid_temp,1,12)==substring(pid_samp,1,12)){
      ppdl1[kk]<-pid_pdl1$'CD274'[jj]
      break
    }
  }
 }
