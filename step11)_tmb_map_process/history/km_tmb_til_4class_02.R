...
# high_high vs high_low vs low_high vs low_low
...
data_path0<-"E:/Hongming/projects/tcga-bladder-mutationburden/Hongming_codes/step11)_tmb_map_process/"
...
for (kk in 1:length(pid)){
  ...
  for (jj in 1:length(pid_tmb_pred$`patient ID`))
    if (temp_pID==substring(as.character(pid_tmb_pred$`patient ID`[jj]),2,24)){
      plabel[kk]<-paste(pid_tmb_pred$predictions[jj],'-',plabel1[kk],sep="")
      break
    }
}
...
ggsurvplot(fit1,pval = TRUE,
           #risk.table = TRUE, 
           legend=c(0.8,0.9),
           legend.labs=c("High-High (73)","High-Low (76)","Low-High (69)","Low-Low (66)"),
