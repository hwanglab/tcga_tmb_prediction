...
for ip=1:length(imagePath)
    ...
    for im=1:numel(imgs)
        ...
        temp=strcmp(blca_MutBurdens(:,1),patientID(1:12));
        catg=blca_MutBurdens(temp==1,3);
        if Indices(im)==2 % for valiation   3: fold1, 2: fold2, 1: fold3
            switch catg{1}
                case 'Low'
                    clusterImgOutput='Y:\DL_TMB_prediction\data_bladder_20X_norm\fold02\validation\low\';
                case 'Mid'
                    clusterImgOutput='Y:\DL_TMB_prediction\data_bladder_20X_norm\fold02\validation\mid\';
                case 'High'
                    clusterImgOutput='Y:\DL_TMB_prediction\data_bladder_20X_norm\fold02\validation\high\';
                otherwise
                    disp('impossible!!!!');
            end
        else
            switch catg{1}
                case 'Low'
                    clusterImgOutput='Y:\DL_TMB_prediction\data_bladder_20X_norm\fold02\training\low\';
                case 'Mid'
                    clusterImgOutput='Y:\DL_TMB_prediction\data_bladder_20X_norm\fold02\training\mid\';
                case 'High'
                    clusterImgOutput='Y:\DL_TMB_prediction\data_bladder_20X_norm\fold02\training\high\';
            end
        end
