# os2 survival plots for the paper
...
if (figa==TRUE){
  ...
  dfs2$fustat <- factor(dfs2$`Overall Survival Status`,
                        levels=c("1:DECEASED","0:LIVING"),
                        labels=c("1","0"))
  
  dfs2$futime<-as.numeric(as.character(dfs2$`Overall Survival (Months)`)) # note that: must be numeric type
