...
plots_km<-function(pid_tils,col_name,blca_data){
  ...
  for (kk in 1:length(plabel)){
    if (is.na(plabel[kk])){
      ...
      pid1[ind]<-pid[kk]
      ind<-ind+1
    } else if ((plabel0[kk]=='Low') & (plabel[kk]=='1')){
      plabel1[ind]<-'LHHL'
      pid1[ind]<-pid[kk]
      ind<-ind+1
      }else {
  ...
  #   if (length(ind)==1) {
  #     stime<-c(stime,as.numeric(blca_data$`Combined days to last followup or death`[ind]))
  #     status<-c(status,as.character(blca_data$`Vital status`[ind]))
  #              
  #   } else {
  #     stime<-c(stime,'NA')
  #     status<-c(status,'NA')
  ...
  # }
  # 
  # sup_tab['Combined days to last followup or death']<-stime
  # sup_tab['Vital status']<-status
