function extract_nuclei_feature_kmeans
...
numc=3;  % number of nuclei clusters
method='kmeans';
fb=10;
for ip=1:length(imagePath)
    ...
    for im=1:numel(imgs)
        ...
        imgseg=dir(fullfile(image_path{ip},strcat(patientID,'*')));
        ...
        for ss=1:numel(imgseg)
            ...
            ab = lab_he(:,:,2:3);
            img2=double(ab);
            ab=reshape(img2,size(img,1)*size(img,2),2);
            % repeat the clustering 3 times to avoid local minima
            rng default; % for reproducibility
            opts=statset('Display','final');
            
            [idx,C] = kmeans(ab,numc,'Replicates',3,'Options',opts);
            pixel_labels=reshape(idx,size(img,1),size(img,2));
            imshow(pixel_labels,[])
            ...
            blue_nuclei = img .* uint8(mask_dark_blue);
            imshow(blue_nuclei)
            title('Blue Nuclei');
