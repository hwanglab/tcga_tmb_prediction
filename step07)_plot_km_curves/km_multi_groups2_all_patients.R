...
for (kk in 1:length(pid)){
  if (plabel[kk]=="High-High-Low") {
    ...
    ind<-ind+1
  } else if (plabel[kk]=="Int-High-Low") {
    pid2[ind]<-pid[kk]
    plabel2[ind]<-plabel[kk]
    ind<-ind+1
  } else if (plabel[kk]=="Int-High-High" | plabel[kk]=="Int-Low-High" | plabel[kk]=="Int-Low-Low"){
    pid2[ind]<-pid[kk]
    plabel2[ind]<-'Int others'
    ind<-ind+1
...
ggsurvplot(fit1,pval = TRUE,
           #risk.table = TRUE, 
           legend=c(0.75,0.85),
           legend.labs=c("High-High-Low (HHL) (42)","High (w/o HHL) (84)","Intermediate-High-Low (IHL) (24)","Intermediate (w/o IHL) (97)",
                         "Low-High-Low (LHL) (19)", "Low (w/o LHL) (102)"),
...
ggsave(filename = "km_4class_all2.eps",
