...
%           svm for linear kernels were used to make predictions
...
load(strcat(matpath,'P_E_TD_linear.mat'));
response_td=labels;
p_e_td=SSC;

load(strcat(matpath,'P_E_RTS_linear.mat'))
response_apc=labels;
p_e_apc=SSC;

load(strcat(matpath,'P_E_CN_linear.mat'))
response_cn=labels;
p_e_cn=SSC;

load(strcat(matpath,'p_inceptionv3_linear.mat')); % bladder cancer tmb
response=labels;
p_inceptionv3=SSC;

load(strcat(matpath,'p_resnet50_linear.mat')); % bladder cancer tmb lbp
response_resnet50=labels;
p_resnet50=SSC;

load(strcat(matpath,'p_xception_linear.mat')); % bladder cancer tmb lbp
...
legend([h1,h2,h3,h4,h5,h6],{'P-E-TD (AUC=0.666)','P-E-CN (AUC=0.719)','P-E-RTS (AUC=0.769)','P-InceptionV3 (AUC=0.645)','P-Resnet50 (AUC=0.691)','P-Xception (AUC=0.748)'});
